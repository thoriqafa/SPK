...
uses SysUtils, ZDataset, Controls, OleCtrls, comobj, Classes, ADODB, Windows, ImageHlp;
...
type
  ...
  private
    ...
    mLevelUser: string;
    mNamaPlant: string;
    mDefaultIdPlant: string;

    mIdPeriode: Integer;
    mPeriode1: TDateTime;
    mPeriode2: TDateTime;
    mPeriodeAktual: string;

    mIdUser: Integer;

    procedure SetPlant(value: string);
    ...
    procedure SetLevel(value: string);
  ...
  public
    ...
    property Plant : string read GetPlant write SetPlant;
    ...
    property HostName: string read mHostName write mHostName;
...
//    procedure crv_preview;
    ...
    property BaudRate: string read mBaudRate write mBaudRate;

    property IdDept: Integer read mIdDept write mIdDept;

    property IdBagian: Integer read mIdBagian write mIdBagian;

    property IdPlant: Integer read mIdPlant write mIdPlant;

    property LevelUser: string read mLevelUser write mLevelUser;

    property NamaPlant: string read mNamaPlant write mNamaPlant;

    function CompareSAP: boolean;

    property DefaultIdPlant: string read mDefaultIdPlant write mDefaultIdPlant;

    procedure TrimMemory;

    property IdPeriode: Integer read mIdPeriode write mIdPeriode;
    property Periode1: TDateTime read mPeriode1 write mPeriode1;
    property Periode2: TDateTime read mPeriode2 write mPeriode2;
    property PeriodeAktual: string read mPeriodeAktual write mPeriodeAktual;

    property IdUser: integer read mIdUser write mIdUser;
...
begin
  try
    ...
    {$ELSE}
    //Self.version := '250321.1649';
    Self.version := FormatDateTime('ddmmyy.hhnn', unTools.LocalDateTimeFromUTCDateTime(Self.LinkerTimestamp));
    mJudulAplikasi := 'PT. Bumi Menara Internusa - [' + Self.Version + ']';
...
function TAplikasi.LinkerTimestamp: TDateTime;
begin
...
procedure TAplikasi.SetLevel(value: string);
begin
  mLevel := value;
end;

function TAplikasi.GetLevel: string;
begin
  Result := mLevel;
end;
...
procedure TAplikasi.SetPlant(value: string);
begin
  mPlant := value;
end;

function TAplikasi.GetPlant: string;
begin
  Result := mPlant;
end;
...
function TAplikasi.CompareSAP: boolean;
var
  q: TZQuery;
begin
  q := OpenRS('SELECT compare_sap FROM tbl_app');
  if q.FieldByName('compare_sap').AsInteger = 1 then
    Result := True
  else
    Result := False;
  q.Close;
...
procedure TAplikasi.TrimMemory;
{
var
  MainHandle : THandle;
begin
