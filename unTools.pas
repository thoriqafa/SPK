...
function OpenRS_RO(sql:string): TZReadOnlyQuery; overload;
function OpenRS_RO(sql:string; Args: array of const): TZReadOnlyQuery; overload;
...
function StrToFloat2(r: Real): real;
  ...
  try
    ...
    begin
      if fs.DecimalSeparator = '.' then
        s := StringReplace(s, '.', ',', [])
      else if fs.DecimalSeparator = '.' then
        s := StringReplace(s, ',', '.', []);
    end;
    r := StrToFloat(s);
  except
...
function OpenRS_RO(sql:string; Args: array of const): TZReadOnlyQuery;
var
  q: TZReadOnlyQuery;
begin
  q := TZReadOnlyQuery.Create(nil);
...
var
  ...
  counter: integer;
begin
  if sTipe = 'mutinout' then
    depan := 'MTS/'
  else if sTipe = 'brg-keluar' then
    depan := 'OT/'
  else if sTipe = 'borongan' then
    depan := 'BOR/'
  else if sTipe = 'lembur' then
    depan := 'OT/'
