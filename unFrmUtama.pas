      ...
      begin
        ...
        begin
          ...
          try
            ...
            begin
              qUploader := OpenRS('SELECT CONCAT(username,'' - '',fullname) AS uploader_name FROM bmi_user WHERE id = %s', [intToStr(Aplikasi.IdUser)]);
              FFileSAP := sPath + sr.Name;
              cxtCurrency.Text := 'RP';
              cxtNoDok.Text := q.FieldByName('no_doc').AsString;
              cxtUploader.Text := qUploader.FieldByName('uploader_name').AsString;
